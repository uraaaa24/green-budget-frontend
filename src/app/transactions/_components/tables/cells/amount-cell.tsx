...
const ExpenseAmountCell = ({ amount }: { amount: number }) => {
  return (
    <span className="rounded-full border-red-400 bg-red-100 px-3 py-2 text-red-800/70">
      -{amount}
    </span>
  )
}
...
const IncomeAmountCell = ({ amount }: { amount: number }) => {
  return (
    <span className="rounded-full border-blue-400 bg-blue-100 px-3 py-2 text-blue-800/70">
      {amount}
    </span>
  )
}
